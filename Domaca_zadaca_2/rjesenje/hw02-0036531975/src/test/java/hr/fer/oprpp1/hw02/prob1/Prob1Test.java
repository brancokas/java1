...
public class Prob1Test {
	...
	public void testExampleTxt1() {
		...
		String doc = "This is sample text.\n"
				...
				+ "{$end$}";
		assertEquals(doc, parser.getDocumentNode().toString());
	}

	@Test
	public void testExampleTxt2() {
		String text = readExample(2);
		SmartScriptParser parser = new SmartScriptParser(text);
		String doc = "{$for i -23 -23.23 @sin_2$}\n"
				+ "	{$= 1$}\n"
				+ "	{$= 123456 i_123 456$}\n"
				+ "	{$for i -23 -23.23 @sin_2$}\n"
				+ "		{$= 1$} {$= 123456 i_123456$} \n"
				+ "	{$end$}\n"
				+ "	{$for variable \"\\n\\t\\r\\\\\" \"\" \"\\\"\\\\\\\"\"$}\n"
				+ "	{$= 1$} {$= 123456 i_123456$}\n"
	...
	@Test
	public void testExampleTxt2Equals() {
		String text = readExample(2);
	...
	@Test
	public void testExampleTxt3() {
		String text = readExample(3);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertThrows(SmartScriptParserException.class, () -> parser.getDocumentNode());
		
	}

	@Test
	public void testExampleTxt4() {
		String text = readExample(4);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertEquals("{$for foric afor bfor$}\\{$for end = \"\"\"\\\\ for$}{$end$}", 
				parser.getDocumentNode().toString());
	}

	@Test
	public void testExampleTxt5() {
	...
	@Test
	public void testExampleTxt8() {
		String text = readExample(8);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertEquals("{$for i int 0 10$}{$end$}"
				, parser.getDocumentNode().toString());
	}

	@Test
	public void testExampleTxt9() {
	...
	@Test
	public void testExampleTxt10() {
		String text = readExample(10);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertThrows(SmartScriptParserException.class, () -> parser.getDocumentNode());
	}
	@Test
	public void testExampleTxt11() {
		String text = readExample(11);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertEquals("{$for forend \"for\" \"end\" endfor$}{$end$}{$= \"=\" sco_re for_end___ \"'''''\"$}"
				, parser.getDocumentNode().toString());
	}
	@Test
	public void testExampleTxt12() {
	...
	@Test
	public void testExampleTxt13() {
		String text = readExample(13);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertEquals("{$= * + - ^ / / / / + + + + + - - - - - "
				+ "-12334 + + 123445 ^ ^ ^ ^ -123.34512 -1232.21 + 214.2 + 21 * + + ^$}"
				, parser.getDocumentNode().toString());
	}
	@Test
	public void testExampleTxt14() {
		String text = readExample(14);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertThrows(SmartScriptParserException.class, () -> parser.getDocumentNode());
	}
	@Test
	public void testExampleTxt15() {
		String text = readExample(15);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertThrows(SmartScriptParserException.class, () -> parser.getDocumentNode());
	}
	@Test
	public void testExampleTxt16() {
		String text = readExample(16);
		SmartScriptParser parser = new SmartScriptParser(text);
		assertEquals("1{$for i -23 -23.23 @sin_2$}{$= 1$}{$= 123456 i_123456$}{$end$}"
				, parser.getDocumentNode().toString());
	}
	...
	private String readExample(int n) {
		  try(InputStream is = this.getClass().getClassLoader().getResourceAsStream("examples/doc"+n+".txt")) {
		    if(is==null) throw new RuntimeException("Datoteka examples/doc"+n+".txt je nedostupna.");
